...
bool FastHash::set(std::string_view key, std::string_view value, std::optional<int> ttl_seconds) {
    std::scoped_lock lock(m_mtx);
    ...
    if (ttl_seconds.has_value()) {
        ...
        if (!m_isAofLoading.load(std::memory_order_relaxed)) {
            m_aofLogger.log(std::format("SETEX {} {} {}", key, *ttl_seconds, value));
        }
    } else {
...
int FastHash::ttl(std::string_view key) {
    ...
    if (auto expiry_opt = m_ttlManager.get_expiry_time(key_str); expiry_opt.has_value()) {
        ...
        auto remaining =
            std::chrono::duration_cast<std::chrono::seconds>(*expiry_opt - now).count();
...
bool FastHash::save_async(std::string_view filename) const {
    ...
    m_saveThread = std::jthread([this, filename = std::string{filename}](std::stop_token token) {
...
json FastHash::serialize() const {
    ...
    for (const auto& [key, val]: m_store) {
        ...
        if (auto expiry_time = m_ttlManager.get_expiry_time(key); expiry_time.has_value()) {
            auto ttl = std::chrono::duration_cast<std::chrono::seconds>(*expiry_time - now).count();
            if (ttl > 0) {
                entry["ttl"] = ttl;
            }
        }
